@tool
class_name OrtGithubDownloader
extends RefCounted
## Editor-only helper for the onnxruntime.gd addon.
##
...
## GitHub Actions artifacts of the onnxruntime.gd repository.
##
...
const REPO_NAME := "onnxruntime.gd"

const ADDON_DIR := "res://addons/onnxruntime.gd"
const EXTENSION_CONFIG_PATH := "res://addons/onnxruntime.gd/gdext.gdextension"
...
static func list_artifacts(names: PackedStringArray) -> Dictionary:
	...
	var headers := PackedStringArray(["User-Agent: onnxruntime.gd-editor-plugin", "Accept: application/vnd.github+json"])
	var found := {}
...
static func download_artifact_zip(artifact_id: int, zip_path: String, progress: Callable = Callable(), total_bytes: int = 0) -> Dictionary:
	...
	var headers := PackedStringArray(["User-Agent: onnxruntime.gd-editor-plugin"])
...
## The zip stores files under a `demo/addons/onnxruntime.gd/` prefix, so entries are matched by name.
