...
## onnxruntime.gd GDExtension libraries.
##
...
func _ready() -> void:
	title = "Manage onnxruntime.gd Libraries"
...
func _build_ui() -> void:
	...
	header.text = "Prebuilt onnxruntime.gd libraries are built by the %s/%s GitHub Actions workflow and downloaded through nightly.link (no GitHub token required). After installing a library, restart the editor to load it." % [Downloader.REPO_OWNER, Downloader.REPO_NAME]
...
func _finish_job() -> void:
	...
	if result.get("ok", false):
		...
		printerr("[onnxruntime.gd] %s" % result.get("error", "unknown error"))
	_downloading = false
	refresh()
	_start_next()
