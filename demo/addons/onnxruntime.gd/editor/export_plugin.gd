...
## Auto-downloads and attaches the onnxruntime.gd GDExtension library for the platform being
...
const Downloader := preload("res://addons/onnxruntime.gd/editor/github_downloader.gd")

var _handled_key := ""


func _get_name() -> String:
	return "onnxruntime.gd"
...
func _export_begin(features: PackedStringArray, is_debug: bool, path: String, flags: int) -> void:
	...
	if result.get("ok", false) and result.get("downloaded", false):
		print("[onnxruntime.gd] Downloaded missing library '%s' (%s) for this export." % [Downloader.library_filename(key), Downloader.format_bytes(result.get("bytes", 0))])
	else:
		...
		_warn("Download it from the onnxruntime.gd library manager (Editor > Manage onnxruntime.gd Libraries...) and export again.")


func _warn(message: String) -> void:
	printerr("[onnxruntime.gd] Warning: " + message)
	var platform := get_export_platform()
	if platform != null:
		platform.add_message(EditorExportPlatform.EXPORT_MESSAGE_WARNING, "onnxruntime.gd", message)
