...
const MENU_ITEM_NAME := "Manage onnxruntime.gd Libraries..."
...
func _enter_tree() -> void:
	_export_plugin = preload("res://addons/onnxruntime.gd/editor/export_plugin.gd").new()
	add_export_plugin(_export_plugin)

	_manager = preload("res://addons/onnxruntime.gd/editor/library_manager.gd").new()
	_manager.hide()
...
func _install_settings_menu_item() -> void:
	var menu := _find_settings_menu()
	if menu == null:
		push_warning("ONNXRuntime.gd: could not locate the Editor menu; the library manager is not available.")
		return
...
func _check_libraries_installed() -> void:
	...
	for key in config.get_section_keys("libraries"):
		...
		if not lib_abs.is_empty() and FileAccess.file_exists(lib_abs):
			return
	push_warning(
		"No ONNXRuntime.gd library is installed. Use Editor > %s to download the library for your platform and/or the platform(s) you plan to export your project to." % MENU_ITEM_NAME
	)
