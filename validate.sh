...
if [ "$DO_BUILD" = "1" ] || [ "$DO_BUILD" = "true" ]; then
    ...
    # --editable: the source tree is regenerated by CMake during configure time on every
    # run if needed, so a cached binary package is always stale. Disables binary caching
    # so vcpkg always does a real (incremental) rebuild of the gdext port.
    "$VCPKG_ROOT/vcpkg" install --editable gdext 2>&1 | tee "$BUILD_LOG"
    ...
    if [ $BUILD_EXIT -ne 0 ]; then
        ...
        if [ -n "$ERROR_FILE" ]; then
            VCPKG_BUILD_DIR="$VCPKG_ROOT/buildtrees/gdext"
            {
                ...
                # build-*-out.log files under buildtrees/gdext/.
                COMPILER_ERRORS_FOUND=false
                for logfile in "$VCPKG_BUILD_DIR"/build-*-out.log; do
                    if [ -f "$logfile" ]; then
                        grep_results=$(grep -n \
    ...
    if [ -n "$EXT_LIB" ]; then
        ...
        if [ -n "$UNDEF" ]; then
            echo "✗ Extension has undefined ORT symbols (header/lib drift?):" >&2
            echo "$UNDEF" >&2
            if [ -n "$ERROR_FILE" ]; then
                {
                    echo "Undefined ORT symbols in the built extension:"
                    echo "$UNDEF"
                } > "$ERROR_FILE"
            fi
            exit 1
